...
namespace Client.DataTests
{
    public class ClientDataAPITests
    {
        ...
        public void RepositoryClients()
        {
            ...
            CClient client1 = new CClient() { ID = 0, Name = "Janusz Patena", Adress = "Zgierz, £Ûdzka 5" };
            repo.ModifyClient(client1);
            Assert.IsTrue(repo.FindClientByID(0).Adress == "Zgierz, £Ûdzka 5");

            // Try to find non existing client
            Assert.IsNull(repo.FindClientByID(4));

            // Create new product and try to add it
            CClient client2 = new CClient() { ID = 4, Name = "Mateusz Zawiadowski", Adress = "Chrzπszczyøewoszyce 5" };
        ...
        private void FillDbContext()
        {
            ...
            repo.AddClient(new CClient() { ID = 1, Name = "Jane Doe", Adress = "£Ûdü, Dolna 10" });
            repo.AddClient(new CClient() { ID = 2, Name = "Hrabia TyczyÒski", Adress = "£Ûdü, WÛlczaÒska 160" });
            repo.AddClient(new CClient() { ID = 3, Name = "Jan Jer", Adress = "£Ûdü, Pomorska 45" });
